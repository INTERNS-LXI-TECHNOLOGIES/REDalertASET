...
export interface IAlertProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export type IAlertState = IPaginationBaseState;
...
export class Alert extends React.Component<IAlertProps, IAlertState> {
  ...
  render() {
    const { alertList, match, totalItems } = this.props;
    return (
      <div>
        <h2 id="alert-heading">
          Alerts
          ...
          <Table responsive>
            ...
            <tbody>
              {alertList.map((alert, i) => (
                <tr key={`entity-${i}`}>
                  <td>
                    <Button tag={Link} to={`${match.url}/${alert.id}`} color="link" size="sm">
                      {alert.id}
                    </Button>
                  </td>
                  <td>{alert.type}</td>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${alert.id}`} color="info" size="sm">
                        <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">View</span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${alert.id}/edit`} color="primary" size="sm">
                        <FontAwesomeIcon icon="pencil-alt" /> <span className="d-none d-md-inline">Edit</span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${alert.id}/delete`} color="danger" size="sm">
